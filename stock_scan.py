...
# ─── 時區處理 ─────────────────────────────────────────────────────
# 美股報告日期應以美國東部時間（ET）為準
def now_us_eastern():
    """Return current time in US Eastern Time (ET) as naive datetime"""
    utc_now = datetime.utcnow()
    # ET = UTC-5 (標準) 或 UTC-4 (夏令時)
    # 為簡化，直接使用 UTC-5 避免時區計算，因為腳本多在收盤後執行
                ...
                try:
                    ...
                    try:
                        ...
                        T_zone = max((datetime.strptime(d.get(f'{k}_expiry', '') or
                                      (now_us_eastern()+timedelta(days=5)).strftime('%Y-%m-%d'),
                                      '%Y-%m-%d') - now_us_eastern()).days, 1) / 252
...
def main():
    ...
    date_str    = now_us_eastern().strftime('%Y%m%d')
